...
if [ -f ${RESULT_DIR}/${SOURCE_NAME}.evec ]
then
	log "Skip PCA analysis (seems to be already done)"
else
	...
	then
		...
		then
			...
			COUNT_CORRUPT=`wc -l ${TEMP_DIR}/05-merge-hapmap.missnp`
			
			log "exluce ${COUNT_CORRUPT} corrupt SNPs (differing alleles between HapMap data and your data"


	                ${PLINK} \
        	                --noweb \
        	                --bfile ${TEMP_DIR}/04-extract-hapmap-flip-snps \
				--exclude ${TEMP_DIR}/05-merge-hapmap.missnp \
                        	--extract ${TEMP_DIR}/02-high-ld.prune.in \
                        	--make-bed \
	                        --out ${TEMP_DIR}/04-extract-hapmap-flip-snps-exclude-corrupt \


			log "merge HapMap - phase 3"

			${PLINK} \
                        	--noweb \
                        	--bfile ${TEMP_DIR}/04-extract-hapmap-flip-snps-exclude-corrupt \
                        	--bmerge ${SCRIPT_DIR}/aux/hapmap3r2_CEU.CHB.JPT.YRI.founders.no-at-cg-snps.bed \
                                	${SCRIPT_DIR}/aux/hapmap3r2_CEU.CHB.JPT.YRI.founders.no-at-cg-snps.bim \
                                	${SCRIPT_DIR}/aux/hapmap3r2_CEU.CHB.JPT.YRI.founders.no-at-cg-snps.fam \
                        	--extract ${TEMP_DIR}/02-high-ld.prune.in \
			...
			mv ${TEMP_DIR}/05-merge-hapmap.log ${LOG_DIR}/hapmap-merge-after-flip-and-exclude.log
		fi
	...
	cat ${SCRIPT_DIR}/aux/convertf.par.template | \
		sed "s|TEMP_DIR|${TEMP_DIR}|g" > ${TEMP_DIR}/07-convertf.par
	${CONVERTF} -p ${TEMP_DIR}/07-convertf.par 2>&1 >/dev/null

	log "conduct PCA"

	cat ${SCRIPT_DIR}/aux/smartpca.par.template | \
		sed "s|TEMP_DIR|${TEMP_DIR}|g" > ${TEMP_DIR}/08-smartpca.par
	${SMARTPCA} -p ${TEMP_DIR}/08-smartpca.par 2>&1 \
		> ${TEMP_DIR}/08-smartpca.stdout.stderr
